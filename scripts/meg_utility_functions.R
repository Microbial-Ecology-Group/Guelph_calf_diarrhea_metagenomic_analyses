...
meg_ordination <- function(data_list,
  ...
  for( l in 1:length(local_obj) ) {
    ...
    #g_ord <- g_ord + scale_fill_tableau("Classic 10", direction = -1)
    #g_ord <- g_ord + scale_color_tableau("Classic 10", direction = -1)
    g_ord <- g_ord +
  ...
  g_all_ord <- g_all_ord +
    ...
    theme(strip.text.x=element_text(size=26),
          ...
          panel.border = element_rect(colour = "black", fill=NA, size=0.5))
  #g_all_ord <- g_all_ord + scale_color_tableau("Classic 10", direction = -1)
  #g_all_ord <- g_all_ord + scale_fill_tableau("Classic 10", direction = -1)  
...
heatmap_col <- c("#f7f6ed","#efdfa7","#f77a74")
...
meg_alpha_diversity <- function(data_list,
  ...
  g_alphadiv <- g_alphadiv + scale_fill_tableau("Classic 10", direction = -1)
  ...
  g_sraw <- ggplot(data=all_species_CSS, aes_string(group_var, 'Value', fill=group_var)) +
    geom_boxplot(size=1) + 
    facet_wrap(~Level, scales='free_y')
  g_sraw <- g_sraw + scale_fill_tableau("Classic 10", direction = -1)
  g_sraw <- g_sraw +
